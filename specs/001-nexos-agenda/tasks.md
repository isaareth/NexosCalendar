...
- [X] T001 Crear el proyecto Next.js 14+ (App Router, TypeScript estricto) en la raíz del
      repositorio con `create-next-app`
- [X] T002 [P] Configurar Tailwind CSS v4 (config CSS-first vía `@theme` en
      `app/globals.css`, sin `tailwind.config.ts`)
- [X] T003 [P] Configurar tipografía editorial (`next/font/google`: "Playfair Display" para
      titulares + "Source Sans 3" para cuerpo) en `app/layout.tsx`, expuesta como
      `--font-heading`/`--font-sans` en `app/globals.css` — Constitución Principio VI, ver
      [research.md §7](research.md#7-tipografía-e-identidad-visual-editorial-principio-vi)
- [X] T004 [P] Inicializar shadcn/ui, re-tematizar (radios más orgánicos, sin sombra tipo
      dashboard) e instalar los componentes base (`button`, `dialog`, `select`, `input`,
      `badge`, `card`) en `components/ui/`
- [X] T005 [P] Instalar dependencias de Supabase (`@supabase/ssr`, `@supabase/supabase-js`)
- [X] T006 [P] Configurar ESLint + Prettier con reglas de TypeScript estricto
- [X] T007 [P] Crear `.env.example` con `NEXT_PUBLIC_SUPABASE_URL`,
      `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`
- [X] T008 [P] Configurar Vitest (`vitest.config.ts`) y Playwright (`playwright.config.ts`)
...
- [X] T012 [P] Crear `lib/theme.ts` con `CATEGORY_COLOR` y `GENDER_COLOR` per
...
- [X] T013 [P] Crear `lib/types.ts` con los tipos `Event` y `EventInput` compartidos
- [X] T014 [P] Crear `lib/validators.ts` con la validación de `EventInput` (reutiliza
      `lib/categories.ts` para la regla género-Fútbol; no la reimplementa)
- [X] T015 [P] Crear `lib/supabase/server.ts` (cliente Supabase para Server
      Components/Actions vía `@supabase/ssr`, basado en cookies)
- [X] T016 [P] Crear `lib/supabase/client.ts` (cliente Supabase para Client Components)
- [X] T017 Crear `proxy.ts` (convención Next.js 16, reemplaza a `middleware.ts`) que protege
      todo bajo `/admin` excepto `/admin/login`,
...
- [X] T018 [P] [US1] Test unitario de `CATEGORY_ORDER`/`genderAppliesTo` en
      `tests/unit/categories.test.ts` (incluye el caso "Fútbol habilita género, el resto no")
- [X] T019 [P] [US1] Test E2E Playwright del flujo público (countdown, filtros, modal) en
      `tests/e2e/public-view.spec.ts`

### Implementation for User Story 1

- [X] T020 [US1] Implementar `listEvents(filters)` y `getNextEvent()` en `lib/queries.ts`
      usando el cliente Supabase de servidor (depende de T015)
- [X] T021 [P] [US1] Componente `Countdown` (Client Component) en
...
- [X] T022 [P] [US1] Componente `CalendarView` (Client Component, vistas mensual/semanal) en
      `app/(public)/components/CalendarView.tsx`
- [X] T023 [P] [US1] Componente `Filters` (categoría/género/carácter) en
...
- [X] T024 [P] [US1] Componente `EventModal` en `app/(public)/components/EventModal.tsx`
      (título, fecha/hora, lugar, carácter, descripción, `extra_info`, `result`)
- [X] T025 [P] [US1] Componente `CharacterBadge` (insignia obligatorio/voluntario) en
      `app/(public)/components/CharacterBadge.tsx`
- [X] T026 [US1] Ensamblar `app/page.tsx` como Server Component: obtiene próximo evento y
...
- [X] T027 [US1] Aplicar estilos responsivos (desktop/tablet/móvil) con Tailwind a los
...
- [X] T028 [P] [US2] Test unitario de `lib/validators.ts` en `tests/unit/validators.test.ts`
      (incluye: carácter requerido, normalización server-side de `gender` a `no_aplica`
      fuera de Fútbol, `end_time >= start_time`)
- [X] T029 [P] [US2] Test E2E Playwright de login + CRUD en `tests/e2e/admin-crud.spec.ts`
      (incluye el intento de acceso a `/admin/dashboard` sin sesión)

### Implementation for User Story 2

- [X] T030 [US2] Página `app/admin/login/page.tsx` con formulario email/password
- [X] T031 [US2] Server Actions `signIn`/`signOut` en `app/admin/login/actions.ts`, usando
      `lib/supabase/server.ts`, con mensaje de error genérico en credenciales inválidas
- [X] T032 [US2] Página `app/admin/dashboard/page.tsx` (Server Component autenticado) que
      lista actividades con acciones editar/eliminar
- [X] T033 [US2] Componente `EventForm` (Client Component) en
...
- [X] T034 [US2] Server Actions `createEvent`/`updateEvent`/`deleteEvent` en
      ...
      T014, T015)
- [X] T035 [US2] Conectar `EventForm` y la lista del dashboard a las Server Actions
      (crear/editar/eliminar) con `revalidatePath` sobre `/` tras cada mutación
- [X] T036 [US2] Agregar estados de error/vacío y diálogo de confirmación de borrado en el
      dashboard
...
- [X] T037 [P] Script/documentación de aprovisionamiento de las 6 cuentas directivas en
      `scripts/seed-directors.md` (pasos manuales en Supabase Auth, según Research §6)
- [X] T038 [P] Revisar/actualizar `README.md` del proyecto raíz (setup, arquitectura, flujo
...
- [X] T040 [P] Pasada de accesibilidad básica (labels, foco visible, contraste) en vistas
      pública y admin, incluyendo el contraste de los colores por categoría (FR-019)
- [X] T041 [P] Auditoría final de tokens de marca: confirmar que ningún componente
